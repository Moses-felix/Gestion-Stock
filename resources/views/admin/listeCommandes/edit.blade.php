        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.listeCommande.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.liste-commandes.update", [$listeCommande->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('commande') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="commande_id" id="commande_id" required>
                                @foreach($commandes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('commande_id') ? old('commande_id') : $listeCommande->commande->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('product') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="product_id" id="product_id" required>
                                @foreach($products as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $listeCommande->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantite') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantite" id="quantite" value="{{ old('quantite', $listeCommande->quantite) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('prix_unitaire') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="prix_unitaire" id="prix_unitaire" value="{{ old('prix_unitaire', $listeCommande->prix_unitaire) }}" step="1" required>
