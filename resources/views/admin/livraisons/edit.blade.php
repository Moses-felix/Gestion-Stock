        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.livraison.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.livraisons.update", [$livraison->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('commande') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="commande_id" id="commande_id" required>
                                @foreach($commandes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('commande_id') ? old('commande_id') : $livraison->commande->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('etat') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="etat" id="etat" value="1" {{ $livraison->etat || old('etat', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('statut') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="statut" id="statut" value="{{ old('statut', $livraison->statut) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('product_category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="product_category_id" id="product_category_id" required>
                                @foreach($product_categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_category_id') ? old('product_category_id') : $livraison->product_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('rangement') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="rangement_id" id="rangement_id" required>
                                @foreach($rangements as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('rangement_id') ? old('rangement_id') : $livraison->rangement->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
