        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.validationAchat.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.validation-achats.update", [$validationAchat->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('users') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="users_id" id="users_id">
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('users_id') ? old('users_id') : $validationAchat->users->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('users'))
                        ...
                        <div class="form-group {{ $errors->has('demande_achat') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="demande_achat_id" id="demande_achat_id" required>
                                @foreach($demande_achats as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('demande_achat_id') ? old('demande_achat_id') : $validationAchat->demande_achat->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('commentaire') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="commentaire" id="commentaire" value="{{ old('commentaire', $validationAchat->commentaire) }}">
                        ...
                        <div class="form-group {{ $errors->has('etat') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\ValidationAchat::ETAT_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="etat_{{ $key }}" name="etat" value="{{ $key }}" {{ old('etat', $validationAchat->etat) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('statut') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="statut" id="statut" value="{{ old('statut', $validationAchat->statut) }}" step="1">
