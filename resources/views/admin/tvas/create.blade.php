        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.tva.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.tvas.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('tva') ? 'has-error' : '' }}">
                            <label class="required" for="tva">{{ trans('cruds.tva.fields.tva') }}</label>
                            <input class="form-control" type="number" name="tva" id="tva" value="{{ old('tva', '0.18') }}" step="0.01" required>
                            @if($errors->has('tva'))
