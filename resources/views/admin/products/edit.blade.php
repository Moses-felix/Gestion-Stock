                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('quantite') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantite" id="quantite" value="{{ old('quantite', $product->quantite) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('categorie') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="categorie_id" id="categorie_id" required>
                                @foreach($categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('categorie_id') ? old('categorie_id') : $product->categorie->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('rangement') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="rangement_id" id="rangement_id" required>
                                @foreach($rangements as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('rangement_id') ? old('rangement_id') : $product->rangement->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
