        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.ligneDemande.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.ligne-demandes.update", [$ligneDemande->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('product') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="product_id" id="product_id" required>
                                @foreach($products as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $ligneDemande->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantite') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantite" id="quantite" value="{{ old('quantite', $ligneDemande->quantite) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('demande') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="demande_id" id="demande_id">
                                @foreach($demandes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('demande_id') ? old('demande_id') : $ligneDemande->demande->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('demande_achat') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="demande_achat_id" id="demande_achat_id">
                                @foreach($demande_achats as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('demande_achat_id') ? old('demande_achat_id') : $ligneDemande->demande_achat->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
